	...
	var charCode, charCode_uh;
 	...
 	for(i=0, k=-1; ue[i]; i++) {
		...
		for(j=0; ue[i][j]; j++) {
			...
			var col = appendChild(row,'div',tdclass,tdid,'','32px','0 0 0 0');
			...
			if(!option.square_layout) {
				if(k==13) col.style.width = '56px'; // backspace
				if(k==14) col.style.width = '50px'; // tab
				if(k==27) col.style.width = '38px'; // \ 글쇠
				if(k==28) col.style.width = '56px'; // shift lock
				if(k==40) col.style.width = '64px'; // Enter
				if(k==41) col.style.width = '70px'; // 왼쪽 shift
				if(k==52) col.style.width = '82px'; // 오른쪽 shift
			}
			else { // 가지런한 배열표
				if(k==0) col.style.width = '56px'; // ` 글쇠
				...
				if(k==41) col.style.width = '56px'; // 왼쪽 shift
				if(k==52) col.style.width = '104px'; // 오른쪽 shift
			}

			if(ue[i][j]=='Back' || ue[i][j]=='Tab' || ue[i][j]=='Enter' || ue[i][j]=='Shift') col.style.textAlign = 'center';

			if(i==4) {
				if(ue[i][j]=='Space') col.style.width = '376px';
				else col.style.width = '32px', col.className = 'e3 special';
			}
